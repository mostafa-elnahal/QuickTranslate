...
dotnet publish ..\src\QuickTranslate\QuickTranslate.csproj -c Release -r win-x64 --self-contained false -p:PublishDir=bin\Release\net8.0-windows\win-x64\publish_light

Write-Host "`nBuilding Standalone (Self-Contained) version..."
dotnet publish ..\src\QuickTranslate\QuickTranslate.csproj -c Release -r win-x64 --self-contained true -p:PublishDir=bin\Release\net8.0-windows\win-x64\publish_standalone
